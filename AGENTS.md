...
- How: use `.opencode/skills/worktree-ux-pr/scripts/upload-pr-artifact.mjs`, store the returned public URLs, and reference those URLs in the PR body or comments. Never commit proof files or PR notes.
...
8. Once the PR exists, upload screenshots/videos to Supabase and embed the returned public URLs in the PR body (only if relevant in the UI).
