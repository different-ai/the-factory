...
async function provisionWorkerOnRender(input: ProvisionInput): Promise<ProvisionedInstance> {
  ...
  const startCommand = [
    ...
    `openwrk serve --workspace /tmp/workspace --openwork-host 0.0.0.0 --openwork-port $PORT --opencode-host 127.0.0.1 --opencode-port 4096 --connect-host 127.0.0.1 --cors '*' --approval manual --no-opencode-router`,
  ].join(" && ")
  ...
  const payload = {
    ...
    serviceDetails: {
      ...
      envSpecificDetails: {
        buildCommand: `npm install -g openwrk@${env.render.workerOpenwrkVersion}`,
        startCommand,
      },
    },
  }
