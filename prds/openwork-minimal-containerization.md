...
## Success metrics
- A local host can be shared via Workspace URL + QR/deeplink without manual token copy/paste in under 2 minutes.
- A remote client can connect to a hosted workspace and run a session without any host-side reconfiguration.
- In sandboxed mode, attempts to access paths outside the allowlist fail reliably and are logged.
- The UI and CLI degrade gracefully based on `GET /capabilities` without hard-coded environment checks.
- At least one deploy adapter (SSH or PaaS) can provision a host in a single command and return a working connect artifact.
...
## Definition of Done
This PRD is done when the following are true in a single release train (not partial experiments).

- Host contract v1 is documented, versioned, and implemented in `openwork-server`.
  - Required endpoints exist and are covered by a capabilities schema with `schemaVersion` and `serverVersion`.
- A connect artifact (`openwork.connect.v1`) is generated by the host, and the UI can connect using QR/deeplink without manual token entry.
- At least one deploy adapter (SSH or PaaS) is implemented end-to-end and returns a working workspace URL.
- Sandbox runtime abstraction exists in `openwrk` with Docker/Podman backend and a no-sandbox fallback.
  - Apple Container backend is optional but supported where available.
- Tool-provider routing is implemented for browser automation and file injection, with capability gating.
- Toy OpenWork UI is served by `openwork-server` and can run sessions, display status/capabilities, and handle approvals.
- Documentation is updated: host contract, deploy adapter usage, sandbox flags, and provider placement modes.

Validation required for DoD (must be executed and recorded in the PR):
- CLI end-to-end: start host in sandboxed mode and run a session through `/opencode/*`.
- REST checks: `GET /health`, `GET /capabilities`, `GET /workspaces` succeed on a fresh host.
- UI E2E (Chrome MCP): connect via Workspace URL, run a session, and approve a write.
- Security check: attempts to access a denied mount path fail in sandboxed mode and are logged.
